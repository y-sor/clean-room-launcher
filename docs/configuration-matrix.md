...
| Claude Code | v0.4.3 whole-plugin selector | `--with=plugin:<provider-native-id>` admits exactly one already-installed plugin whose observed effective surface is skill-only | Exact real-provider E2E on Claude Code 2.1.280 / macOS Apple Silicon |
...
settings, and notifications are not activated. By contrast, the v0.4.3
