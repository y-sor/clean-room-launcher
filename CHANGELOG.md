...
- Advanced exact macOS Apple Silicon release qualification to current stable
  Codex `0.155.1` and Claude Code `2.1.280`.
...
- Move the final remote `main`/tag/ruleset/release-contract refresh after provider action-time checks so the protected tag push has the smallest practical TOCTOU window.
